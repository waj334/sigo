...
struct AttachDebugInfoPass : PassWrapper<AttachDebugInfoPass, OperationPass<ModuleOp>> {
  ...
  void runOnOperation() override final {
    ...
    module.walk([&](func::FuncOp op) {
      ...
      if (const auto fusedLocWithCompileUnit =
        ...
        const auto subroutineTypeAttr = LLVM::DISubroutineTypeAttr::get(context, llvm::dwarf::DW_CC_normal, {});
        const auto subprogramAttr = LLVM::DISubprogramAttr::get(
            context, subprogramIdAttr, compileUnitAttr, fileAttr, baseFuncNameAttr, funcNameAttr, fileAttr,
            /*line=*/loc.getLine(),
            /*scopeline=*/loc.getLine(), LLVM::DISubprogramFlags::Definition | LLVM::DISubprogramFlags::Optimized,
            subroutineTypeAttr);
    ...
    module.walk([&](GlobalOp op) {
      const auto fusedLocWithCompileUnit = op->getLoc()->findInstanceOf<mlir::FusedLocWith<LLVM::DICompileUnitAttr>>();
  ...
  void processAllocOp(AllocaOp op, OpBuilder &builder, DataLayout &dataLayout, RuntimeTypeLookUp &runtimeTypes) {
    ...
    if (name && !(*name).empty()) {
      const auto fusedLoc = op->getParentOp()->getLoc()->findInstanceOf<FusedLocWith<LLVM::DISubprogramAttr>>();
      if (!fusedLoc)
        return;
      ...
      const auto diFile = LLVM::DIFileAttr::get(context, path.filename().string(), path.parent_path().string());
      const auto diLocalVarAttr = LLVM::DILocalVariableAttr::get(scope, *name, diFile,
                                                                 loc.getLine(),                  // LINE
                                                                 0,                              // ARG,
  ...
  LLVM::DITypeAttr getDITypeAttr(MLIRContext *context, Type type, const DataLayout &dataLayout,
    ...
    if (const auto namedType = mlir::dyn_cast<NamedType>(type); namedType) {
      const auto underlyingType =
          getDITypeAttr(context, namedType.getUnderlying(), dataLayout, runtimeTypes, namedType.getName());
      return LLVM::DIDerivedTypeAttr::get(context, llvm::dwarf::DW_TAG_typedef, namedType.getName(), underlyingType,
                                          size, align, 0, std::nullopt, LLVM::DINodeAttr());
    }
    ...
    case GoTypeId::Bool:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "bool", 1, llvm::dwarf::DW_ATE_boolean);
      break;
    case GoTypeId::Int:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "int", size, llvm::dwarf::DW_ATE_signed);
      break;
    case GoTypeId::Int8:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "char", size, llvm::dwarf::DW_ATE_signed);
      break;
    case GoTypeId::Int16:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "short", size, llvm::dwarf::DW_ATE_signed);
      break;
    case GoTypeId::Int32:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "int", size, llvm::dwarf::DW_ATE_signed);
    ...
    case GoTypeId::Uint64:
      result = LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "unsigned long long int", size,
                                          llvm::dwarf::DW_ATE_unsigned);
      break;
    ...
    case GoTypeId::Float32:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "float", size, llvm::dwarf::DW_ATE_float);
      break;
    case GoTypeId::Float64:
      result =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "double", size, llvm::dwarf::DW_ATE_float);
      break;
    ...
    case GoTypeId::Array: {
      ...
      result = LLVM::DICompositeTypeAttr::get(
          ...
          0, // LINE
          diScope, getDITypeAttr(context, arrayType.getElementType(), dataLayout, runtimeTypes), LLVM::DIFlags::Zero,
          size, align, {diSubrange});
      break;
    }
    case GoTypeId::Chan:
    ...
    case GoTypeId::Func: {
      ...
      result = LLVM::DIDerivedTypeAttr::get(context, llvm::dwarf::DW_TAG_pointer_type, StringAttr::get(context, ""),
                                            diSignature, size, align, 0, std::nullopt, LLVM::DINodeAttr());
      break;
    }
    case GoTypeId::Interface:
      result = getDITypeAttr(context, runtimeTypes.lookupRuntimeType("interface"), dataLayout, runtimeTypes);
      break;
    case GoTypeId::Map:
    ...
    case GoTypeId::Struct: {
      ...
      this->m_typeMap[type] = mlir::cast<LLVM::DICompositeTypeAttr>(LLVM::DICompositeTypeAttr::getRecSelf(recId));
      ...
      for (size_t i = 0; i < numElements; i++) {
        ...
        if (fieldNameAttr.str() != "_") {
          ...
          const auto derivedTypeAttr =
              LLVM::DIDerivedTypeAttr::get(context, llvm::dwarf::DW_TAG_member, fieldNameAttr, elementTypeAttr,
                                           elementSize, alignment, offset, std::nullopt, LLVM::DINodeAttr());
      ...
      result = LLVM::DICompositeTypeAttr::get(context, llvm::dwarf::DW_TAG_structure_type, recId, nameAttr, diFile,
                                              0, // LINE
                                              diScope, LLVM::DINullTypeAttr::get(context), LLVM::DIFlags::Zero, size,
                                              align, elementAttrs);
    } break;
    case GoTypeId::UnsafePointer: {
      const auto base =
          LLVM::DIBasicTypeAttr::get(context, llvm::dwarf::DW_TAG_base_type, "void", 0, llvm::dwarf::DW_ATE_unsigned);
      result =
          LLVM::DIDerivedTypeAttr::get(context, llvm::dwarf::DW_TAG_pointer_type, StringAttr::get(context, "void*"),
                                       base, size, align, 0, std::nullopt, LLVM::DINodeAttr());
    } break;
    default:
      return {};
    }
