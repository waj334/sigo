...
namespace mlir::go {
struct GlobalConstantsPass : public mlir::PassWrapper<GlobalConstantsPass, mlir::OperationPass<mlir::ModuleOp>> {
public:
  void runOnOperation() final {
    ...
    module.walk([&](ConstantOp constOp) {
      if (go::isa<StringType>(constOp.getType())) {
        ...
        auto it = std::find_if(globalStrings.begin(), globalStrings.end(),
    ...
    // Create the global constant strings
    {
      ...
      for (const auto &[str, loc] : globalStrings) {
        ...
        globalGoStrOp = builder.create<mlir::LLVM::GlobalOp>(loc, _stringType, true, mlir::LLVM::Linkage::External,
                                                             name, Attribute(), 0);
        ...
        // Init block
        {
          ...
          // Position the build inside of the init block
          builder.setInsertionPointToStart(initBlock);

          // Create the global C-string
          name = "cstr_" + std::to_string(strHash);
          auto globalCStr = mlir::LLVM::createGlobalString(loc, builder, name, str, mlir::LLVM::Linkage::External);
          ...
          mlir::Value lenVal = builder.create<mlir::LLVM::ConstantOp>(loc, builder.getI32Type(),
                                                                      builder.getI32IntegerAttr(str.length()));
  ...
  void createGlobals(mlir::ModuleOp module, const LLVMTypeConverter &typeConverter, OpBuilder &builder) {
    mlir::OpBuilder::InsertionGuard guard(builder);
    ...
    for (auto [symbol, op] : globalOps) {
      ...
      // TODO: Alignment needs to be specified to avoid issues on platforms that do not allow unaligned memory access.
      auto globalOp = builder.create<mlir::LLVM::GlobalOp>(op.getLoc(), resultType, false,
                                                           mlir::LLVM::Linkage::External, op.getSymName(), Attribute());

      if (op->hasAttr("llvm.debug.global_expr")) {
        auto globalExpr = mlir::cast<LLVM::DIGlobalVariableExpressionAttr>(op->getAttr("llvm.debug.global_expr"));
      ...
      if (op->hasAttr("llvm.linkage")) {
        if (auto linkage = mlir::cast<LLVM::LinkageAttr>(op->getAttr("llvm.linkage"));
          ...
          // Explicit external global should not be considered for initialization as externally linked objects
          // are expected to satisfy this dependency.
      ...
      if (shouldInitialize) {
        ...
        // TODO: The usages of the global should be analyzed. The package init methods contain initializers
        //       for these globals and the foldable and materializable operations could be moved here in
        //       theory.
...
std::unique_ptr<mlir::Pass> createGlobalConstantsPass() { return std::make_unique<GlobalConstantsPass>(); }
} // namespace mlir::go
