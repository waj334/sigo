...
namespace mlir::go {
    struct CallPass : public mlir::PassWrapper<CallPass, mlir::OperationPass<mlir::ModuleOp> > {
        ...
        void runOnOperation() final {
            ...
            auto createGeneralCallThunk = [&](OpBuilder &builder, FunctionType fnT, Type argPackType) -> std::string {
                mlir::OpBuilder::InsertionGuard guard(builder);
                builder.setInsertionPointToStart(module.getBody());

                // Note: Since thunks can be reused for calls with matching signature, no fixed location for the
                ...
                // Look up the thunk symbol by hash value and create the function if it is not found in the cache.
                if (const auto it = this->m_thunkSymbols.find(argsHash); it == this->m_thunkSymbols.end()) {
                    ...
                    auto funcOp = builder.create<func::FuncOp>(loc, funcSymbolName, signature);
                    ...
                    // Build the function body.
                    {
                        ...
                        for (int i = 0; i < callArgs.size(); ++i) {
                            ...
                            Value callArgPtr = builder.create<GetElementPointerOp>(loc,
                                ptrType, argsPtr, argPackType, ValueRange{}, SmallVector{0, i + 1});
                            callArgs[i] = builder.create<LoadOp>(loc, argType, callArgPtr, UnitAttr(), UnitAttr());
                        }

                        // Call the function being wrapped.
                        ValueRange results = builder.create<CallIndirectOp>(loc,
                                                                            fnT.getResults(), funcPtr,
                                                                            callArgs).getResults();
            ...
            auto createInterfaceCallThunk = [&](OpBuilder &builder, FunctionType fnT, Type interfaceType,
                                                StringRef callee,
                                                ValueRange args,
                ...
                if (const auto it = this->m_thunkSymbols.find(argsHash); it == this->m_thunkSymbols.end()) {
                    ...
                    // Build the function body.
                    {
                        ...
                        Value interfaceValue = builder.create<LoadOp>(loc, interfaceType, argsPtr,
                                                                      UnitAttr(), UnitAttr());
                        ...
                        for (int i = 0; i < args.size(); ++i) {
                            ...
                            Value callArgPtr = builder.create<GetElementPointerOp>(
                                loc, ptrType, argsPtr, argPackType, ValueRange{}, SmallVector{0, i + 1});
                            callArgs[i] = builder.create<LoadOp>(loc, argType, callArgPtr, UnitAttr(), UnitAttr());
                        }

                        // Call the function being wrapped.
                        auto results = builder.create<InterfaceCallOp>(loc,
                                                                       TypeRange{}, callee, interfaceValue,
                                                                       callArgs).getResults();
                ...
                TypeSwitch<Type>(calleeType)
                        .Case([&](const FunctionType T) {
                            if (T.getNumInputs() > 0) {
                                ...
                                const auto argPackType = LLVM::LLVMStructType::getLiteral(context, argTypes);

                                // Find or create a thunk to wrap the callee so that it's call arguments can be unpacked
                                // properly.
                                ...
                                Value args = rewriter.create<AllocaOp>(loc, ptrType, argPackType,
                                                                       Value(), UnitAttr(), StringAttr());
                                ...
                                for (int i = 0; i < callArgs.size(); ++i) {
                                    ...
                                    Value addr = rewriter.create<GetElementPointerOp>(
                                        loc, ptrType, args, argPackType, ValueRange{}, SmallVector{0, i + 1});
                        ...
                        .Case([&](LLVM::LLVMStructType T) {
                            if (!callArgs.empty()) {
                                // Collect the expected argument pack struct member types.
                                // NOTE: The pointer to the callee is the first argument and the second is the arguments
                                ...
                                const auto syntheticFnT = FunctionType::get(context, inputs, signature.getResults());
                                ...
                                Value addr = rewriter.create<GetElementPointerOp>(loc,
                                    ptrType, args, argPackType, ValueRange{}, SmallVector{0, 1});
                                ...
                                for (int i = 0; i < callArgs.size(); ++i) {
                                    ...
                                    addr = rewriter.create<GetElementPointerOp>(
                                        loc, ptrType, args, argPackType, ValueRange{}, SmallVector{0, i + 2});
                                    rewriter.create<StoreOp>(loc, arg, addr, UnitAttr(), UnitAttr());
                                }
                        ...
                        .Case([&](InterfaceType T) {
                            ...
                            // Pack the call arguments.
                            for (int i = 0; i < callArgValues.size(); ++i) {
                                auto arg = callArgValues[i];
                                Value addr = rewriter.create<GetElementPointerOp>(
                                    loc, ptrType, args, argPackType, ValueRange{}, SmallVector{0, i});
                                rewriter.create<StoreOp>(loc, arg, addr, UnitAttr(), UnitAttr());
                            }
                        ...
                        .Default([&](Type) {
                            assert(false && "unhandled callee type");
                        });
    ...
    std::unique_ptr<mlir::Pass> createCallPass() { return std::make_unique<CallPass>(); }
}
