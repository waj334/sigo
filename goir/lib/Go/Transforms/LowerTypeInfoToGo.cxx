...
namespace mlir::go {
    ...
    private:
        GlobalOp createTypeInfo(mlir::OpBuilder &builder, const LLVMTypeConverter &converter, const mlir::Location &loc,
            ...
            const auto extraData = this->m_typeDataMap[T];
            ...
            globalDataOp =
                    ...
                    .Case<InterfaceType>([&](InterfaceType interfaceType) {
                        ...
                        for (auto [name, func]: methods) {
                            ...
                            const auto interfaceMethodSymbol =
                                    typeInfoSymbol(interfaceType,
                                                   "_iface_method_" + name + "_" + std::to_string(id) + "_");
                            auto interfaceMethodOp =
                                    createGlobal(builder, interfaceMethodDataType, interfaceMethodSymbol, loc,
                                                 [&](OpBuilder &builder) {
                            ...
                            Value methodPtrValue =
                                    builder.create<
                                        AddressOfOp>(loc, interfaceMethodPtr, interfaceMethodOp.getSymName());
                        ...
                        return createGlobal(builder, sliceType, dataSymbol, loc, [&](OpBuilder &builder) {
                            ...
                            auto sliceValue = this->createSlice(builder, methodsSymbol, interfaceMethodPtr, methodsData,
                                                                loc);
                    ...
                    .Case<NamedType>([&](NamedType namedType) {
                            ...
                            if (methods && !methods.empty()) {
                                ...
                                for (const auto &method: methods) {
                                    ...
                                    const auto funcDataSymbol =
                                            typeInfoSymbol(
                                                T, "_" + typeName.str() + "_named_method_" + baseMethodName.str() + "_"
                                                   +
                                                   std::to_string(id) + "_");
                                    auto funcDataGlobalOp =
                                            createGlobal(builder, funcDataType, funcDataSymbol, loc,
                                                         [&](OpBuilder &builder) {
                                                             ...
                                                             Value funcPtrValue = builder.create<AddressOfOp>(
                                                                 loc, ptrType, funcSymbol);
                                                             Value signatureInfoValue =
                                                                     builder.create<AddressOfOp>(
                                                                         loc, infoPtrType, signatureInfo.getSymName());
                                                             ...
                                                             funcDataValue = builder.create<InsertOp>(
                                                                 loc, funcDataType, constId, 0, funcDataValue);
                                                             funcDataValue = builder.create<InsertOp>(
                                                                 loc, funcDataType, funcPtrValue, 1, funcDataValue);
                                                             funcDataValue =
                                                                     builder.create<InsertOp>(
                                                                         loc, funcDataType, signatureInfoValue, 2,
                                                                         funcDataValue);
                        ...
                        return createGlobal(builder, namedDataType, namedTypeSymbol, loc, [&](OpBuilder &builder) {
                            ...
                            Value sliceVal = createSlice(builder, funcsArraySymbol, funcDataPtrType, funcDataValues,
                                                         loc);

                            // Create the final named type data struct
                            Value namedTypeDataValue = builder.create<ZeroOp>(loc, namedDataType);
                            namedTypeDataValue =
                                    builder.create<InsertOp>(loc, namedDataType, underlyingTypeDataValue, 0,
                                                             namedTypeDataValue);
                            namedTypeDataValue = builder.create<InsertOp>(
                                loc, namedDataType, sliceVal, 1, namedTypeDataValue);
        ...
        GlobalOp createGlobal(mlir::OpBuilder &builder, mlir::Type type, const std::string &symbol,
            ...
            globalOp->setAttr("llvm.linkage",
                              mlir::LLVM::LinkageAttr::get(&this->getContext(), mlir::LLVM::Linkage::External));
        ...
        GlobalOp createUninitializedGlobal(mlir::OpBuilder &builder, mlir::Type type, const std::string &symbol,
            ...
            op->setAttr("llvm.linkage",
                        mlir::LLVM::LinkageAttr::get(builder.getContext(), mlir::LLVM::Linkage::External));
            return op;
        }
        ...
        Value createSlice(OpBuilder &builder, const std::string &symbol, Type elementType, ValueRange values,
                          Location loc) {
