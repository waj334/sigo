...
namespace mlir::go
{
struct LowerTypeInfoPass
  : public mlir::PassWrapper<LowerTypeInfoPass, mlir::OperationPass<mlir::ModuleOp>>
{
  ...
  void runOnOperation() final
  {
    ...
    // Create the globals
    {
      ...
      for (auto [typeHash, values] : typesMap)
      {
        ...
        createTypeInfo(builder, module, location, type);
      }
    }
  }
...
private:
  GlobalOp createTypeInfo(mlir::OpBuilder &builder, const LLVMTypeConverter &converter, const
mlir::Location &loc, mlir::Type T, const DenseMap<Type, DictionaryAttr>& typeData = DenseMap<Type,
DictionaryAttr>()) { MLIRContext *context = builder.getContext(); const auto extraData =
typeData.contains(T) ? typeData.at(T) : DictionaryAttr();
      ...
      globalDataOp =
              ...
              .Case<ArrayType>([&](ArrayType arrayType) {
                  ...
                  return createGlobal(builder, arrayDataType, dataSymbol, loc, [&](OpBuilder
&builder) { auto typeSymbol = typeInfoSymbol(arrayType.getElementType());

                      mlir::Value lenVal =
                              builder.create<ConstantOp>(loc, ui16Type,
                                                         builder.getI64IntegerAttr(arrayType.getLength()));
                      mlir::Value typeVal = builder.create<AddressOfOp>(loc, infoPtrType,
typeSymbol);
              ...
              .Case<ChanType>([&](ChanType chanType) {
                  ...
                  // Create the global struct
                  return createGlobal(builder, chanDataType, dataSymbol, loc, [&](OpBuilder
&builder) { auto typeSymbol = typeInfoSymbol(chanType.getElementType()); mlir::Value typeInfoVal =
builder.create<AddressOfOp>(loc, infoPtrType, typeSymbol); mlir::Value dirVal =
builder.create<ConstantOp>( loc, ui8Type,
builder.getI64IntegerAttr(static_cast<uint8_t>(chanType.getDirection())));

                      mlir::Value dataVal = builder.create<ZeroOp>(loc, chanDataType);
                      dataVal = builder.create<InsertOp>(loc, chanDataType, typeInfoVal, 0,
dataVal); dataVal = builder.create<InsertOp>(loc, chanDataType, dirVal, 1, dataVal);
                      builder.create<YieldOp>(loc, dataVal);
                  });
              })
              .Case<FunctionType>([&](FunctionType funcType) {
                  const auto dataSymbol = typeInfoSymbol(funcType, "_signature_");
                  return createGlobal(builder, signatureDataType, dataSymbol, loc, [&](OpBuilder
&builder) { size_t offset = 0;
                      ...
                      if (extraData && extraData.getAs<BoolAttr>("receiver").getValue()) {
                          // The receiver can be known contextually from the first parameter of the
function signature data. const auto receiverType = funcType.getInput(0); createTypeInfo(builder,
converter, loc, receiverType);

                          auto typeSymbol = typeInfoSymbol(receiverType);
                          receiverTypeVal = builder.create<AddressOfOp>(loc, infoPtrType,
typeSymbol); offset = 1; } else { receiverTypeVal = builder.create<ZeroOp>(loc, infoPtrType);
                      }
                      ...
                      Value paramsSliceVal = this->createSlice(builder, paramsSymbol, infoPtrType,
                                                               inputInfoValues, loc);
                      ...
                      for (size_t i = 0; i < funcType.getNumResults(); ++i) {
                          ...
                          resultInfoValues[i] = builder.create<AddressOfOp>(loc, infoPtrType,
GV.getSymName());
                      }

                      // Create the slice value holding the parameters.
                      const auto resultsSymbol = typeInfoSymbol(funcType, "_signature_results_");
                      Value resultsSliceVal = this->createSlice(builder, resultsSymbol, infoPtrType,
                                                                resultInfoValues, loc);

                      // Create the signature data
                      Value signatureDataVal = builder.create<ZeroOp>(loc, signatureDataType);
                      signatureDataVal =
                              builder.create<InsertOp>(loc, signatureDataType, receiverTypeVal, 0,
                                                       signatureDataVal);
                      signatureDataVal =
                              builder.create<InsertOp>(loc, signatureDataType, paramsSliceVal, 1,
                                                       signatureDataVal);
                      signatureDataVal =
                              builder.create<InsertOp>(loc, signatureDataType, resultsSliceVal, 2,
                                                       signatureDataVal);
              ...
              .Case<InterfaceType>([&](InterfaceType interfaceType) {
                  ...
                  // Create the type information for the interface method signatures referenced
below. SmallVector<Value> methodsData(methods.size()); size_t counter = 0; for (auto [name, func]:
methods) { auto signatureInfoOp = this->createTypeInfo(builder, converter, loc, func); const auto id
= computeMethodHash(name, func, true); const auto interfaceMethodSymbol =
                              typeInfoSymbol(interfaceType,
                                             "_iface_method_" + name + "_" + std::to_string(id) +
"_"); auto interfaceMethodOp = createGlobal(builder, interfaceMethodDataType, interfaceMethodSymbol,
loc,
                                           [&](OpBuilder &builder) {
                                               // Compute the id hash of this method.

                                               const Value signatureInfoValue =
                                                       builder.create<AddressOfOp>(
                                                           loc, signatureDataPtrType,
                                                           signatureInfoOp.getSymName());

                                               // Create constant length value.
                                               Value constId = builder.create<ConstantOp>(
                                                   loc, ui32Type, builder.getI64IntegerAttr(id));

                                               // Create the data struct.
                                               Value interfaceMethodValue = builder.create<ZeroOp>(
                                                   loc, interfaceMethodDataType);
                                               interfaceMethodValue =
                                                       builder.create<InsertOp>(
                                                           loc, interfaceMethodDataType, constId, 0,
                                                           interfaceMethodValue);
                                               interfaceMethodValue = builder.create<InsertOp>(
                                                   loc, interfaceMethodDataType, signatureInfoValue,
                      ...
                      Value methodPtrValue =
                              builder.create<
                                  AddressOfOp>(loc, interfaceMethodPtr,
interfaceMethodOp.getSymName()); methodsData[counter++] = methodPtrValue;
                  }
                  ...
                  return createGlobal(builder, sliceType, dataSymbol, loc, [&](OpBuilder &builder) {
                      ...
                      auto sliceValue = this->createSlice(builder, methodsSymbol,
interfaceMethodPtr, methodsData, loc); builder.create<YieldOp>(loc, sliceValue);
                  });
              })
              .Case<NamedType>([&](NamedType namedType) {
                  ...
                  auto underlyingTypeDataOp = createTypeInfo(builder, converter, loc,
underlyingType);

                  if (extraData) {
                      // Create the method data slice.
                      const auto methods = extraData.getAs<ArrayAttr>("methods");
                      if (methods && !methods.empty()) {
                          ...
                          for (const auto &method: methods) {
                              ...
                              auto funcOp =
cast_or_null<func::FuncOp>(getOperation().lookupSymbol(funcSymbol));

                              // Note: It is possible for some method functions to be dropped if
there is no usage of them. Do not
                              ...
                              auto signatureInfo = this->createTypeInfo(builder, converter, loc,
signature);

                              // Exclude the qualifiers from the method name.
                              auto baseMethodName = funcSymbol.getValue();
                              baseMethodName =
baseMethodName.substr(baseMethodName.find_last_of(".") + 1);

                              // Compute the method hash for this struct method.
                              const auto id = computeMethodHash(baseMethodName, signature, false);

                              // Create the global struct
                              const auto funcDataSymbol =
                                      typeInfoSymbol(
                                          T, "_" + typeName.str() + "_named_method_" +
baseMethodName.str() + "_"
                                             +
                                             std::to_string(id) + "_");
                              auto funcDataGlobalOp =
                                      createGlobal(builder, funcDataType, funcDataSymbol, loc,
                                                   [&](OpBuilder &builder) {
                                                       // Get the function pointer using the Go
Dialect's AddressOf operation. This will be lowered
                                                       // during the LLVM lowering pass.
                                                       Value funcPtrValue =
builder.create<AddressOfOp>( loc, ptrType, funcSymbol); Value signatureInfoValue =
                                                               builder.create<AddressOfOp>(
                                                                   loc, infoPtrType,
signatureInfo.getSymName());
...
builder.getI64IntegerAttr(id));
                                                       ...
                                                       Value funcDataValue = builder.create<ZeroOp>(
                                                           loc, funcDataType);
                                                       funcDataValue = builder.create<InsertOp>(
                                                           loc, funcDataType, constId, 0,
funcDataValue); funcDataValue = builder.create<InsertOp>( loc, funcDataType, funcPtrValue, 1,
funcDataValue); funcDataValue = builder.create<InsertOp>( loc, funcDataType, signatureInfoValue, 2,
                                                                   funcDataValue);
                  ...
                  return createGlobal(builder, namedDataType, namedTypeSymbol, loc, [&](OpBuilder
&builder) { const Value underlyingTypeDataValue = builder.create<AddressOfOp>(loc, infoPtrType,
underlyingTypeDataOp.getSymName());

                      SmallVector<Value> funcDataValues(numMethods);
                      for (size_t i = 0; i < numMethods; ++i) {
                          Value funcDataAddr =
                                  builder.create<AddressOfOp>(loc, funcDataPtrType,
                                                              funcDataGlobals[i].getSymName());
                          funcDataValues[i] = funcDataAddr;
                      }

                      // Create the function data slice.
                      const auto funcsArraySymbol = typeInfoSymbol(T, "_" + typeName.str() +
"_methods_arr_"); Value sliceVal = createSlice(builder, funcsArraySymbol, funcDataPtrType,
funcDataValues, loc);

                      // Create the final named type data struct
                      Value namedTypeDataValue = builder.create<ZeroOp>(loc, namedDataType);
                      namedTypeDataValue =
                              builder.create<InsertOp>(loc, namedDataType, underlyingTypeDataValue,
0, namedTypeDataValue); namedTypeDataValue = builder.create<InsertOp>( loc, namedDataType, sliceVal,
1, namedTypeDataValue); builder.create<YieldOp>(loc, namedTypeDataValue);
                  });
              })
...
{ auto keyTypeSymbol = typeInfoSymbol(mapType.getKeyType()); auto valueTypeSymbol =
typeInfoSymbol(mapType.getValueType());

                      mlir::Value keyTypeInfoVal = builder.create<AddressOfOp>(loc, ptrType,
keyTypeSymbol); mlir::Value valueTypeInfoVal = builder.create<AddressOfOp>(loc, ptrType,
valueTypeSymbol);

                      mlir::Value dataVal = builder.create<ZeroOp>(loc, mapDataType);
                      dataVal = builder.create<InsertOp>(loc, mapDataType, keyTypeInfoVal, 0,
dataVal); dataVal = builder.create<InsertOp>(loc, mapDataType, valueTypeInfoVal, 1, dataVal);
                      builder.create<YieldOp>(loc, dataVal);
                  });
              })
      ...
      initGlobal(builder, resultOp, [&]() {
          ...
          auto kindType = NamedType::get(context, ui8Type, kindTypeStrAttr);
          Value kindValue = builder.create<ConstantOp>(loc, kindType,
                                                       builder.getI64IntegerAttr(static_cast<uint8_t>(kind)));
          ...
          Value sizeValue = builder.create<ConstantOp>(loc, ui16Type,
builder.getI64IntegerAttr(size));
          ...
          // NOTE: Padding byte is at index 1.
          infoValue = builder.create<InsertOp>(loc, infoType, sizeValue, 2, infoValue);

          if (globalDataOp) {
              Value dataValue = builder.create<AddressOfOp>(loc, ptrType,
globalDataOp.getSymName()); infoValue = builder.create<InsertOp>(loc, infoType, dataValue, 3,
infoValue);
          }
  ...
  GlobalOp createGlobal(mlir::OpBuilder &builder, mlir::Type type, const std::string &symbol,
      ...
      globalOp->setAttr("llvm.linkage",
                        mlir::LLVM::LinkageAttr::get(&this->getContext(),
mlir::LLVM::Linkage::External));
  ...
  GlobalOp createUninitializedGlobal(mlir::OpBuilder &builder, mlir::Type type, const std::string
&symbol, const mlir::Location &loc) { mlir::OpBuilder::InsertionGuard guard(builder);
...
mlir::LLVM::Linkage::External)); return op;
  }
  ...
  Value createSlice(OpBuilder &builder, const std::string &symbol, Type elementType, ValueRange
values, Location loc) { const auto siT = IntType::get(&this->getContext()); const auto runtimeSliceT
= this->typeConverter->lookupRuntimeType("slice"); const auto sliceT =
SliceType::get(&this->getContext(), elementType); const auto arrT =
ArrayType::get(&this->getContext(), elementType, values.size()); const auto ptrT =
PointerType::get(&this->getContext(), std::nullopt);
      ...
      if (!values.empty()) {
          ...
          auto arrGV = this->createGlobal(builder, arrT, symbol, loc, [&](OpBuilder &builder) {
              ...
              for (auto value: values) {
                  ...
                  // Move the defining operation of the value into the current block before the
insert operation.
          ...
          Value lenV = builder.create<ConstantOp>(loc, siT,
builder.getI64IntegerAttr(values.size())); Value sliceV = builder.create<ZeroOp>(loc,
runtimeSliceT);
...
{
  return std::make_unique<LowerTypeInfoPass>();
}
} // namespace mlir::go
