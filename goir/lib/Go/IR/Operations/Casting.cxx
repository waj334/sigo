...
namespace mlir::go {
    ::mlir::LogicalResult BitcastOp::verify() {
        ...
        // Verify that primitive types are only cast between their MLIR and runtime representations and that pointers
        // are only cast to and from unsafe.Pointer.
        if (failed(llvm::TypeSwitch<mlir::Type, LogicalResult>(this->getValue().getType())
            ...
            .Case([&](NamedType T) {
                bool isSuccess = llvm::StringSwitch<bool>(T.getName().getValue())
            ...
            .Case([&](PointerType T) {
                if (auto resultType = mlir::dyn_cast<PointerType>(this->getType())) {
                    ...
                    if (go::isa<mlir::FunctionType>(this->getType())) {
                        // unsafe.Pointer -> func
                        return success();
                    }
                }
            ...
            return this->emitOpError() << "invalid cast from " << this->getValue().getType() << " to " << this->
                   getType();
        }
        return success();
    }
    ...
    ::mlir::LogicalResult ChangeInterfaceOp::verify() {
        ...
        for (auto &method: resultMethods) {
            ...
            if (sourceMethod == sourceMethods.end()) {
                return this->emitOpError() << "the resulting interface type is missing method \"" << method.first
                       << "\" from the input interface value type";
            }

            // The signatures of the methods MUST match for the resulting interface type to be compatible with the input
            // interface.
            if (method.second != sourceMethod->second) {
                return this->emitOpError() << "the signature of method \"" << method.first
    ...
    LogicalResult TypeAssertOp::canonicalize(TypeAssertOp op, PatternRewriter &rewriter) {
        ...
        auto func = mlir::cast<func::FuncOp>(module.lookupSymbol("runtime.interfaceAssert"));
        ...
        // Handle 'ok' semantics.
        Value hasOk = rewriter.create<ConstantOp>(loc, boolType,
                                                  IntegerAttr::get(boolType, op.getNumResults() == 2 ? 1 : 0));
        ...
        if (mlir::isa<InterfaceType>(op.getType(0))) {
            ...
            // Create the true block.
            {
                ...
                auto interfaceValueCallOp = rewriter.create<RuntimeCallOp>(loc, results, "runtime.interfaceValue", args);
                Value underlyingValuePtr = interfaceValueCallOp->getResult(0);

                // Load the value.
                auto newAssertedValue = rewriter.create<LoadOp>(loc, assertedType, underlyingValuePtr, UnitAttr(),
                                                                UnitAttr());
            ...
            // Terminate the orignal block with a conditional branch.
            {
                ...
                rewriter.create<CondBranchOp>(loc, assertCallOp.getResult(1),
