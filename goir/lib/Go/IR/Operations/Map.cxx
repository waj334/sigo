...
namespace mlir::go {
    LogicalResult MapUpdateOp::canonicalize(MapUpdateOp op, PatternRewriter &rewriter) {
        ...
        rewriter.replaceOpWithNewOp<RuntimeCallOp>(op, results, "runtime.mapUpdate", args);
        return success();
    }
    ...
    LogicalResult MapLookupOp::canonicalize(MapLookupOp op, PatternRewriter &rewriter) {
        ...
        auto lookupOp = rewriter.create<RuntimeCallOp>(loc, results, "runtime.mapUpdate", args);
        ...
        // Split the current block so a conditional branch can be inserted to cover the map lookup semantics.
        ...
        // Terminate the original block with the conditional branch.
        {
            ...
            rewriter.create<CondBranchOp>(loc, ok,
