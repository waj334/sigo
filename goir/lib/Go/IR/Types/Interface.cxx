...
namespace mlir::go {
    namespace detail {
        InterfaceTypeStorage::InterfaceTypeStorage(const std::string &name) : m_name(name) {}

        InterfaceTypeStorage::InterfaceTypeStorage(const FunctionMap &methods) : m_methods(methods) {}
        ...
        InterfaceTypeStorage::construct(::mlir::TypeStorageAllocator &allocator, const KeyTy &key) {
            if (!key.m_name.empty()) {
                return new(allocator.allocate<InterfaceTypeStorage>())
                        InterfaceTypeStorage(key.m_name);
            } else {
                return new(allocator.allocate<InterfaceTypeStorage>())
                        InterfaceTypeStorage(key.m_methods);
            }
        }

        LogicalResult InterfaceTypeStorage::mutate(TypeStorageAllocator &allocator,
    ...
    InterfaceType InterfaceType::get(::mlir::MLIRContext *context,
                ...
                if (succeeded(p.parseOptionalString(&methodName))) {
                    if (methodName.empty()) {
                        p.emitError(p.getCurrentLocation(),
                                    "method name cannot be empty string");
                        return {};
                    }
                    ...
                    if (_methods.find(methodName) != _methods.end()) {
                        p.emitError(p.getCurrentLocation(),
                                    "method called \"" + methodName + " appears more than once");
                        return {};
                    }
    ...
    mlir::LogicalResult InterfaceType::setMethods(const detail::InterfaceTypeStorage::FunctionMap &methods) {
    ...
    ::llvm::TypeSize InterfaceType::getTypeSizeInBits(const DataLayout &dataLayout,
                                                      DataLayoutEntryListRef params) const {
        // The interface type consists of two pointers
        return dataLayout.getTypeSizeInBits(IndexType::get(getContext())) * 2;
    }

    ::llvm::TypeSize InterfaceType::getTypeSize(const DataLayout &dataLayout,
                                        DataLayoutEntryListRef params) const {
        // The interface type consists of two pointers
        return dataLayout.getTypeSize(IndexType::get(getContext())) * 2;
    }

    uint64_t InterfaceType::getABIAlignment(const DataLayout &dataLayout,
                                            DataLayoutEntryListRef params) const {
        return dataLayout.getTypeABIAlignment(IndexType::get(getContext()));
    }

    uint64_t InterfaceType::getPreferredAlignment(const DataLayout &dataLayout,
                                         DataLayoutEntryListRef params) const {
