...
mlir::Type GoStructType::parse(mlir::AsmParser& p)
{
  ...
  if (const auto result = p.parseCommaSeparatedList(
...
llvm::TypeSize GoStructType::getTypeSize(
  ...
  mlir::DataLayoutEntryListRef params) const
{
  ...
  for (size_t i = 0; i < this->getNumFields(); i++)
  {
    ...
    const auto memberTypeAlignment = dataLayout.getTypeABIAlignment(memberType);
    const llvm::Align memberAlignment(memberTypeAlignment);
    ...
    const auto memberSize = dataLayout.getTypeSize(memberType).getFixedValue();
    size += memberSize;
  }
...
llvm::TypeSize GoStructType::getTypeSizeInBits(
...
uint64_t GoStructType::getABIAlignment(const DataLayout& dataLayout, DataLayoutEntryListRef params)
  const
{
  assert(this->getImpl()->m_complete && "struct type must be complete");
...
uint64_t GoStructType::getPreferredAlignment(
  const DataLayout& dataLayout,
  DataLayoutEntryListRef params) const
{
