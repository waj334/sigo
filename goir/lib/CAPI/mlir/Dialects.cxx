...
uint64_t mlirGoGetTypeId(MlirType type) { return mlir::go::getTypeId(unwrap(type)); }
...
MlirLogicalResult mlirGoOptimizeModule(MlirModule module, MlirStringRef name, MlirStringRef outputDir) {
    ...
    pm.addPass(mlir::go::createDumpToFilePass(_name.concat(".core").str(), _outputDir));
...
MlirAttribute mlirGoCreateComplexNumberAttr(MlirContext context, MlirType type, double real, double imag) {
    auto _context = unwrap(context);
    auto _type = unwrap(type);
    auto floatType = _type.cast<mlir::ComplexType>().getElementType();
