...
func (ht *HashTrieMap[K, V]) initSlow() {
	...
	// NOTE(sigo): abi.Type does not store an Equal function pointer.
	// valEqual is only used by CompareAndSwap/CompareAndDelete which
	// guard against nil, so this is safe for the unique package use case.
	ht.valEqual = nil
...
// runtime_rand returns a pseudorandom uint64.
// This is a simplified implementation for SiGo that does not
// depend on runtime.rand via go:linkname.
