...
func (p Pin) SetInterrupt(mode int, handler func(Pin)) {
...
func (p Pin) SetDirection(dir int) {
	portgroup := &chip.PORT.GROUP[0xFF&(p>>8)]
	if dir == 0 {
...
func (p Pin) SetPullMode(mode int) {
	portgroup := &chip.PORT.GROUP[0xFF&(p>>8)]
	if (1<<(p&0xFF))&portgroup.DIR.GetDIR() == 0 {
		if mode == 1 {
			p.Set(false)
