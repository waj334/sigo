			...
			if obj.Parent() == obj.Pkg().Scope() {
				...
				{ // Start constant body block.
					...
					if valueSpec.Values != nil {
						result = b.emitExpr(ctx, valueSpec.Values[i])[0]
					} else {
						value := fromObj(obj)
						constOp := mlir.GoCreateConstantOperation(b.ctx, value, nil, T, location)
						appendOperation(ctx, constOp)
						result = resultOf(constOp)
					}
				...
				constOp := mlir.GoCreateGlobalConstantOperation(b.ctx, nil, b.strAttr(symbolName), location)
				...
				b.setAddr(ctx, ident, ConstantValue{
					Emitter: func(ctx context.Context, location mlir.Location) mlir.Value {
						...
						constRefOp := mlir.GoCreateConstantOperation(b.ctx, value, nil, T, location)
						appendOperation(ctx, constRefOp)
						return resultOf(constRefOp)
					},
					T: T,
